      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="grid gap-4">
          {items.map((faq, i) => (
            <details
              key={i}
              className="group rounded-2xl border bg-background/50 p-6 backdrop-blur-xl transition-all duration-300 open:bg-background/80 open:shadow-md hover:bg-background/80 hover:border-primary/40"
            >
              <summary className="flex cursor-pointer list-none items-center gap-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 [&::-webkit-details-marker]:hidden">
                ...
                <h3 className="flex-1 text-base font-semibold tracking-tight text-foreground sm:text-lg">{faq.q}</h3>
                <ChevronDown className="h-4 w-4 shrink-0 text-muted-foreground transition-transform duration-300 group-open:rotate-180" />
              </summary>
              <p className="mt-3.5 pl-9 text-sm sm:text-base leading-relaxed text-foreground/80 font-normal">{faq.a}</p>
            </details>
          ))}
        </div>
      </div>
